...
class AuthService {
    async reg(email: string, username: string, password: string) {
        ...
        const tokens = await TokenService.generateTokens({userId: user._id,email, username})
        return {user, tokens}
    }
    ...
    async login(dto: inDataToLogin) {
        ...
        const tokens = await TokenService.generateTokens({userId: user._id, email: dto.email, username: dto.username})
        return tokens
    }
